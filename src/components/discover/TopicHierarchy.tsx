...
export function TopicHierarchy({ currentPath, currentTopic, loading: topicLoading, rootCategory }: TopicHierarchyProps) {
  ...
  return (
    <>
      ...
      {matchedCategory && (
        <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 h-[75vh] pt-20 flex flex-col">
          ...
          {/* Content */}
          <div className="relative mx-auto max-w-7xl w-full px-4 py-8 sm:px-6 lg:px-8 flex flex-col flex-1">
            <div className="mb-8">
              <Button
                onClick={() => navigate('/discover')}
                variant="ghost"
              ...
              <p className="text-xl text-white/95 max-w-2xl drop-shadow-md mb-6">
            ...
            {/* Error State - Overlaid on hero */}
            {error && (
              <div className="rounded-lg border border-red-300/50 bg-red-500/20 backdrop-blur-md p-6 text-center mb-8">
                <p className="text-white font-medium">{error}</p>
                <Button onClick={loadSubtopics} className="mt-4 bg-white/20 hover:bg-white/30 text-white border-white/20">
                  Try Again
                </Button>
              </div>
            )}

            {/* Loading State - Skeleton Grid - Overlaid on hero */}
            {subtopicsLoading && (
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 flex-1">
                {[...Array(6)].map((_, idx) => (
                  <div
                    key={idx}
                    className="animate-pulse rounded-xl bg-white/10 backdrop-blur-md border border-white/20 shadow-lg p-6"
                  >
            ...
            {/* Subtopics Grid - Overlaid on hero with backdrop blur */}
            {!subtopicsLoading && !error && subtopics.length > 0 && (
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 flex-1 pb-8">
                {subtopics.map((topic, idx) => (
                  ...
                  >
                    <button
                      ...
                      className="group relative w-full rounded-xl bg-white/10 backdrop-blur-md border border-white/20 shadow-lg hover:shadow-xl hover:bg-white/15 p-6 text-left transition-all duration-300 hover:scale-[1.02] hover:-translate-y-1"
                    >
                      {/* Gradient Accent Border on Hover */}
                      <div className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-br from-blue-400/20 via-purple-400/20 to-pink-400/20 pointer-events-none" />

                      {/* Content */}
                      <div className="relative">
                        {/* Topic Name */}
                        <h3 className="text-lg font-semibold text-white mb-3 group-hover:text-blue-200 transition-colors leading-tight">
                          {topic.name}
                        </h3>

                        {/* Article Count Badge */}
                        {topic.article_count > 0 && (
                          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-sm text-white/80 bg-white/10 group-hover:bg-white/20 transition-colors">
                      ...
                      {/* Hover Effect Line */}
                      <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left" />
                    </button>
                  </div>
                ))}
              </div>
            ...
            {!subtopicsLoading && !error && subtopics.length === 0 && (
              <div className="text-center py-16 flex-1 flex flex-col items-center justify-center">
                ...
                <p className="text-white/80 mb-4">No subtopics available for this topic yet.</p>
                <Button onClick={handleOverviewClick} className="bg-white/10 hover:bg-white/20 text-white border-white/20">
