...
export async function synthesizeNews(request: SynthesisRequest): Promise<NewsData> {
  try {
    console.log('Calling Supabase edge function for:', request.topic);
    
    const { data, error } = await supabase.functions.invoke('synthesize-news', {
      body: {
        topic: request.topic,
        targetOutlets: request.targetOutlets,
        freshnessHorizonHours: request.freshnessHorizonHours,
    ...
    console.log('Successfully received news data:', data);
