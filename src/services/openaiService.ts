  ...
  try {
    ...
    const resp = await openai.responses.create({
      ...
      required: [
        ...
        'topicHottness','summaryPoints','sourceAnalysis',
        'disagreements','article','keyQuestions','sources','missingSources'
      ],
      properties: {
        topic:            { type: 'string' },
        ...
        topicHottness:    { enum: ['High','Medium','Low'] },
        summaryPoints:    { type: 'array', items: { type: 'string' } },
        sourceAnalysis: {
          type: 'object',
          required: ['narrativeConsistency','publicInterest'],
          properties: {
            narrativeConsistency: {
              type: 'object',
        ...
        disagreements: {
          ...
          items: {
            type: 'object',
            required: ['pointOfContention','details','likelyReason'],
            properties: {
              pointOfContention: { type: 'string' },
              details:           { type: 'string' },
        ...
        article: {
          type: 'object',
          required: ['base','eli5','middleSchool','highSchool','undergrad','phd'],
          properties: {
            base:        { type: 'string' },
            eli5:        { type: 'string' },
        ...
        sources: {
          type: 'array',
          minItems: 4, maxItems: 4,                  // enforce exactly 4
          items: {
            type: 'object',
            required: ['id','outlet','type','url','headline','publishedAt','analysisNote'],
            properties: {
              id:          { type: 'string' },
              outlet:      { type: 'string' },
              type:        { type: 'string' },
